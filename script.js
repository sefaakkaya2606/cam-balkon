...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!window.firebase) {
    alert("Firebase yüklenemedi. İnternet bağlantısını ve index.html’deki Firebase script’lerini kontrol edin.");
    return;
  }
  ...
  async function hesapla(){
    ...
    const aluKgFireli = aluKg * (1 + (await getPrices()).firePct/100);

    const A = (await getPrices()).acc;
    let aksesuarTop = 0;
    ...
    aksesuarTop += ((acilim==="yandan"?2:4)*2) * A.pss1;
    const kenetAdetAks = (acilim==="yandan") ? (n-2)*2 : 8;
    aksesuarTop += kenetAdetAks*A.pss2 + kenetAdetAks*A.pss3;
  ...
  el.btnAdminLogin.addEventListener("click", async () => {
    ...
    try{
      ...
      console.error(e); alert("Sunucu hatası.");
    }
  });
