/* =========================================================
   Kasetli Sürme Cam Balkon — script.js (Firebase senkronlu)
   ========================================================= */

/* ---------- Firebase ayarların (DEĞİŞTİR) ---------- */
const firebaseConfig = {
  apiKey: "YOUR_KEY",
  authDomain: "YOUR_DOMAIN.firebaseapp.com",
  databaseURL: "https://YOUR_DB_ID-default-rtdb.europe-west1.firebasedatabase.app",
...
// Veritabanı yolları
const PATH_CFG   = "/app/config";
const PATH_ADM   = "/app/admins";
...
const el = {
  ...
  kilitOrtadan: $("kilitOrtandan"), // güvenlik için null-check
...
/* ===== Config/Admin yükleme (ilk kurulum) ===== */
let CFG = null;
let ADMINS = null;

async function ensureRemoteState(){
  // Config
  CFG = await dbGet(PATH_CFG);
  if (!CFG){
    CFG = DEFAULTS;
    await dbSet(PATH_CFG, CFG);
  }
  // Admins
  ADMINS = await dbGet(PATH_ADM);
  if (!ADMINS){
    ADMINS = { admin: "3482" };
    await dbSet(PATH_ADM, ADMINS);
  }
}
...
function hesapla(){
  ...
  aksesuarTop += ((acilim==="yandan"?2:4)*2) * A.pss1; // çekme kol aksesuarı
  const kenetAdet2 = (acilim==="yandan") ? (n-2)*2 : 8;
  aksesuarTop += kenetAdet2 * A.pss2;
  aksesuarTop += kenetAdet2 * A.pss3;

  function kilitFiyati(ad){
    if (ad==="Klips Kol") return A.pss10;
    if (ad==="Multi Kilit" || ad==="Anahtarlı Kilit") return A.pss9;
...
el.btnAdminLogin.addEventListener("click", async ()=>{
  ...
  const admins = await dbGet(PATH_ADM);
  if (admins && admins[u] && admins[u]===p){ enterAdmin(u); }
...
el.btnSavePrices.addEventListener("click", async ()=>{
  ...
  await dbSet(PATH_CFG, CFG);
  alert("Kaydedildi (tüm cihazlara senkron).");
  refreshAluBadge();
});
...
el.btnChangePin.addEventListener("click", async ()=>{
  ...
  await dbSet(PATH_ADM, admins);
  el.newPin.value = "";
  alert("PIN güncellendi (tüm cihazlarda).");
});
...
el.btnAddAdmin.addEventListener("click", async ()=>{
  ...
  alert("Yönetici eklendi (tüm cihazlarda).");
});
...
/* ===== Başlangıç: uzaktan yükle, sonra UI kur ===== */
(async function init(){
  await ensureRemoteState();        // cfg + admins hazırla/çek
  const u = getSessionUser();
  if (u && (await dbGet(PATH_ADM))?.[u]) enterAdmin(u); else leaveAdmin();
  refreshAcilimKisit();
  refreshAluBadge();
})();
